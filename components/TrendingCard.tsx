...
const TrendingCard = ({
  ...
  // Get the latest isBookmarked value from context
  const currentVideo = videos.find((v) => v.id === id);
  const currentIsBookmarked = currentVideo ? currentVideo.isBookmarked : false;
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      // Always sync with backend after any bookmark change
      await syncBookmarks();
    } catch (err) {
      console.error("Failed to update bookmark in backend", err);
    }
  };
